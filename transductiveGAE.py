...
flags.DEFINE_string('model', 'transGAE', 'Model string.')  # 'attTransGAE', 'transGAE'
flags.DEFINE_float('learning_rate', 0.01, 'Initial learning rate.')
flags.DEFINE_integer('epochs', 200, 'Number of epochs to train.')
# flags.DEFINE_integer('hidden0', 100, 'Number of units in hidden layer 0.')
flags.DEFINE_integer('hidden1', 64, 'Number of units in hidden layer 1.') #dimension of the node embeddings
...
for epoch in range(FLAGS.epochs):
    ...
    # Training step
    outs = sess.run([model.opt_op, model.loss, model.accuracy, model.activations], feed_dict=feed_dict)
...
test_preds = np.add(test_preds, test_preds.T)
testsubs = np.where(test_mask>0)
roc = roc_auc_score(y[testsubs], test_preds[testsubs])
prauc = average_precision_score(y[testsubs], test_preds[testsubs])
# print("Test set results:", "cost=", "{:.5f}".format(test_cost),
#       "accuracy=", "{:.5f}".format(test_acc), "time=", "{:.5f}".format(test_duration))
print("TransGAE_mult", roc, prauc)
